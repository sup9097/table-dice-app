...
class TableDicePredictor:
    ...
    def aggregate_table(self, target_table):
        combined = []
        source_tables = ["2f", "1-1", "1-2", "1-3", "1-4"]
    ...
    def predict_with_model(self):
        ...
        for i in range(2):
            prediction = self.models[self.current_table].predict(last)[0]
            prediction = tuple(int(x) for x in prediction)
            predicted_sum = sum(prediction)
            print(f"🤖 머신러닝 예측 {i+1}: {prediction}")
        ...
        last = np.array([self.histories[self.current_table][-1]])
        prediction = self.models[self.current_table].predict(last)[0]
        print(f"🤖 머신러닝 예측: {tuple(int(x) for x in prediction)}")
